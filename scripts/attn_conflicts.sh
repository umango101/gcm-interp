...
N_TOTAL=$(( ${#selected[@]} * ${#pairs[@]} * ${#algos[@]} * ( ${#arms[@]} + N_EVAL_CELLS ) ))
...
for model_id in "${selected[@]}"; do
    ...
    for pair in "${pairs[@]}"; do
        ...
        for algo in "${algos[@]}"; do
            ...
            # ---- localization: once per ARM, not once per pair ------------ #
            # Each arm is its own corpus, so each needs its own attribution
            # map. --data_dir names the directory; --source/--base still name
            # the file prefixes inside it.
            for arm in "${arms[@]}"; do
                [[ $STOPPED -eq 1 ]] && break
                check_data "$arm" || continue
                run_step "${model_name}__${arm}__${source}_to_${base}__${algo}__patch" \
                         "[$(( N_DONE + 1 ))/$N_TOTAL] $model_name | $arm | $algo | localize" \
                         --model_id "$model_id" \
            ...
            for eval_arm in "${arms[@]}"; do
                ...
                for steer_arm in "${steer_arms[@]}"; do
                    ...
                    for tf in "${test_files[@]}"; do
                        ...
                        run_step "${model_name}__${eval_arm}__${source}_to_${base}__${algo}__eval_${tf}__steer_${steer_arm}" \
                                 "[$(( N_DONE + 1 ))/$N_TOTAL] $model_name | eval:$eval_arm/$tf steer:$steer_arm | $algo" \
                                 --model_id "$model_id" \
                                 --batch_size "$batch_size" \
                                 --patch_algo "$algo" \
                                 --data_dir "$eval_arm" \
