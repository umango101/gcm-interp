...
# no extraversion-* directories built.
check_data() {
    ...
    for fmt in "${formats[@]}"; do
        needed+=(
            "$DATA_ROOT/$model_name/extraversion-$fmt/introversion-$fmt-test.jsonl"
            "$DATA_ROOT/$model_name/extraversion-$fmt/extraversion-$fmt-steering.jsonl"
            "$DATA_ROOT/$model_name/extraversion-$fmt/introversion-$fmt-steering.jsonl"
        )
    done
    local -a missing=()
    local f
        ...
        for algo in "${algos[@]}"; do
            ...
            for eval_fmt in "${formats[@]}"; do
                ...
                for steer_fmt in "${formats[@]}"; do
                    [[ $STOPPED -eq 1 ]] && break
                    eval_test="$DATA_ROOT/$model_name/extraversion-$eval_fmt/introversion-$eval_fmt-test.jsonl"
                    steer_add="$DATA_ROOT/$model_name/extraversion-$steer_fmt/extraversion-$steer_fmt-steering.jsonl"
                    steer_sub="$DATA_ROOT/$model_name/extraversion-$steer_fmt/introversion-$steer_fmt-steering.jsonl"
