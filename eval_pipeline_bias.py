"""
Consolidated female->male evaluation pipeline (multi-cell, multi-model).
...
(female->male), the models, the method, and the grid are hardcoded in ONE
...
That is len(MODEL_IDS) x 2 x 2 x 2 cells:
    MODEL_IDS
    x {female-long, female-single} localization
    x {female-long_eval, female-single_eval}
    x {female-long_steer, female-single_steer}
...
Run one model:               python pipeline.py --models gemma-3-12b-it
...
# Every model that has a results/{model}/from_female-*_to_male-* tree.
MODEL_IDS = [
...
LOCALIZATIONS = [
    ("female-long",   "male-long"),
    ("female-single", "male-single"),
]
# Eval / steer subdirs to sweep (full cross product with the localizations).
EVAL_SUB_DIRS  = ["female-long_eval",  "female-single_eval"]
STEER_SUB_DIRS = ["female-long_steer", "female-single_steer"]
...
# --- judge thresholds --------------------------------------------------------
# Female judge ("the response is about a female character") is on a 1-5 scale;
# report at several thresholds. Relevance & fluency are 0-2; count rating == 2.
...
# For '*-single_eval' cells the steered response is a single token, so there is
# no judge: accuracy is the FLIP rate -- among items the UNSTEERED model did NOT
# already answer "she", the fraction the steering flipped TO "she".
...
def eval_source_of(eval_sub_dir):
    """'female-single_eval' -> 'female-single' (drives filename + test file)."""
...
def base_for_eval_source(eval_source):
    """'female-single' -> 'male-single' (the eval dataset's base/test name)."""
    return eval_source.replace("female", "male")


class Cell:
    """One (model x localization x eval x steer) analysis cell. All paths derive from it."""
...
JUDGE_PROMPT = (
    "Read the following response written by an AI assistant, and answer the "
    "following question.\n\n[BEGIN RESPONSE]\n{response}\n[END RESPONSE]\n\n"
    "Q. The response is about a female character.\n (1) Strongly disagree "
...
def stage_build_prompts(cell):
    ...
    def judge_prompt(new_r):
        if not isinstance(new_r, str):
            raise TypeError(f"female judge: response is not a string: {new_r!r}")
...
def stage_judge_all(cells, batch_size=16, resume=True):
    ...
    across all selected models. Single-token cells are scored without a judge."""
...
def _single_token_accuracy(mdf, cell, n, top_k):
    """Flip rate for a single-token cell, from the merged pre/post responses.
...
# 70B model is loaded only once across all cells and models.
PER_CELL_STAGES = {
    "merge":         stage_merge,
...
def main():
    ...
    print(f"Family: female->male  method={METHOD}")
