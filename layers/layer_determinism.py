...
def env_fingerprint(config):
    ...
    return {
        ...
        # This branch has no --no_deterministic flag: determinism is gated by the
        # env the launcher exports, which eval.setup.assert_determinism_env()
        # checks at startup. Record the env itself so a diff between two runs can
        # be traced to a launcher that forgot an export.
        'PYTHONHASHSEED': os.environ.get('PYTHONHASHSEED'),
        'TOKENIZERS_PARALLELISM': os.environ.get('TOKENIZERS_PARALLELISM'),
        'STRICT_DETERMINISM': os.environ.get('STRICT_DETERMINISM'),
...
def write_fingerprint(config, out_dir, name='determinism.json'):
    ...
    missing = [k for k, v in (('CUBLAS_WORKSPACE_CONFIG', ':4096:8'),
                              ('PYTHONHASHSEED', '0'),
                              ('TOKENIZERS_PARALLELISM', 'false'))
               if fp.get(k) != v]
    if missing:
        print(f"[determinism] WARNING: {missing} not exported by the launcher. "
              f"Generation is nondeterministic; arms produced with and without "
              f"them are not comparable.")
