...
#SBATCH -t 06:00:00
#SBATCH -J biasGenerate_experiment
...
declare -a pairs=(
  "femaleGenerate-long_maleGenerate-long"
  "femaleGenerate-single_maleGenerate-single"
)
...
check_data() {
    ...
    for fmt in "${formats[@]}"; do
        needed+=(
            "$DATA_ROOT/$model_name/femaleGenerate-$fmt/maleGenerate-$fmt-test.jsonl"
            "$DATA_ROOT/$model_name/femaleGenerate-$fmt/femaleGenerate-$fmt-steering.jsonl"
            "$DATA_ROOT/$model_name/femaleGenerate-$fmt/maleGenerate-$fmt-steering.jsonl"
        )
    done
    local -a missing=()
    local f
        ...
        for algo in "${algos[@]}"; do
            ...
            for eval_fmt in "${formats[@]}"; do
                ...
                for steer_fmt in "${formats[@]}"; do
                    [[ $STOPPED -eq 1 ]] && break
                    eval_test="$DATA_ROOT/$model_name/femaleGenerate-$eval_fmt/maleGenerate-$eval_fmt-test.jsonl"
                    steer_add="$DATA_ROOT/$model_name/femaleGenerate-$steer_fmt/femaleGenerate-$steer_fmt-steering.jsonl"
                    steer_sub="$DATA_ROOT/$model_name/femaleGenerate-$steer_fmt/maleGenerate-$steer_fmt-steering.jsonl"
