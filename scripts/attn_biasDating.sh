...
#SBATCH -t 48:00:00
#SBATCH -J biasDating_experiment
...
declare -a pairs=(
  "femaleDating-long_maleDating-long"
  "femaleDating-single_maleDating-single"
)
...
check_data() {
    ...
    for fmt in "${formats[@]}"; do
        needed+=(
            "$DATA_ROOT/$model_name/femaleDating-$fmt/maleDating-$fmt-test.jsonl"
            "$DATA_ROOT/$model_name/femaleDating-$fmt/femaleDating-$fmt-steering.jsonl"
            "$DATA_ROOT/$model_name/femaleDating-$fmt/maleDating-$fmt-steering.jsonl"
        )
    done
    local -a missing=()
    local f
        ...
        for algo in "${algos[@]}"; do
            ...
            for eval_fmt in "${formats[@]}"; do
                ...
                for steer_fmt in "${formats[@]}"; do
                    [[ $STOPPED -eq 1 ]] && break
                    eval_test="$DATA_ROOT/$model_name/femaleDating-$eval_fmt/maleDating-$eval_fmt-test.jsonl"
                    steer_add="$DATA_ROOT/$model_name/femaleDating-$steer_fmt/femaleDating-$steer_fmt-steering.jsonl"
                    steer_sub="$DATA_ROOT/$model_name/femaleDating-$steer_fmt/maleDating-$steer_fmt-steering.jsonl"
