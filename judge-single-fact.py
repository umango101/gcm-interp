...
tasks = ["from_lying-long_to_truthful-long", "from_lying-single_to_truthful-single"]
methods = ['atp']
...
def calculate_accuracy(file_path, base):
    ...
    for item in data:
        # print(base, item)
        if base == 'truthful-single' or base == 'truthful-long': # added the or statement, not sure how differs from line 25
            if 'b' in item[f'old_{base}'].lower():
                total += 1
                if 'a' in item[f'edit_{base}'].lower(): # what does the no do?
                    correct += 1
            elif 'a' in item[f'old_{base}'].lower():
                total += 1
...
for task in tasks:
    ...
    for method in methods:
        ...
        for eval_dir, steering_dir in [('lying-single_eval', 'lying-long_steer'), ('lying-single_eval', 'lying-single_steer')]:
