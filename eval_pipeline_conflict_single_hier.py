...
# Cells are DISCOVERED from disk rather than enumerated here. The hierarchy
# corpora multiply the old single cell by four axes at once -- arm (devuser /
# sysuser / sysdev), method directory (atp / random-uniform /
# random-layer_matched), eval test file (dev-single-test / devNaive-single-test)
# and steering arm (the 3x3 cross-arm grid) -- and the random arms add a fifth,
# the seed, which lives inside the filename as reps=random-s{seed}. Writing that
# product out by hand is how a cell silently goes unscored.
#
# Discovery also handles the ablation for free: mean-interchange runs write
# `_mean_` in the same directory as `_steer_`, so they are a value of the
# STEER_METHOD axis rather than a separate tree.
#
# Everything below is a FILTER on what was found, not a specification of what
# must exist. Empty means no filter.
ARMS = []            # e.g. ["devuser"]
METHODS = []         # e.g. ["atp", "random-layer_matched"]
EVAL_TESTS = []      # e.g. ["dev-single-test"]
STEER_ARMS = []      # e.g. ["devuser"]
REPS_FILTER = []     # e.g. ["targeted"] or ["random-s1", "random-s2"]
STEER_METHODS = []   # e.g. ["steer"] or ["mean"]
...
NS = None
TOP_KS = None
# STEER_METHOD and REPS are now per-cell, carried on the Cell object. The gen
# filename is {N}_{reps}_{steer|mean}_{topk}_{arm}_gen.json, so both are
# recoverable from disk, and a run that mixed reps or ablation types in one
# directory -- which is exactly what the ablation does -- stays separable.
...
GEN_RE = re.compile(
    r"^(?P<N>\d+)_(?P<reps>[A-Za-z0-9\-]+)_(?P<steer_method>steer|mean)_"
    r"(?P<topk>\d+(?:\.\d+)?)_(?P<test_dataset>[A-Za-z0-9\-]+)_gen\.json$"
)
LOC_RE = re.compile(r"^(?P<arm>[^_]+)__from_(?P<source>[^_]+)_to_(?P<base>[^_]+)$")


def _test_stem(eval_sub_dir, arm):
    """'devuser-devNaive-single-test_eval' -> 'devNaive-single-test'.

    config.py builds this component as f"{arm_dir}-{test_file_stem}", so the arm
    prefix is stripped rather than parsed: the stem itself contains hyphens and
    splitting on them would truncate it.
    """
    if not eval_sub_dir.endswith("_eval"):
        raise ValueError(f"eval dir must end with '_eval': {eval_sub_dir}")
    stem = eval_sub_dir[: -len("_eval")]
    prefix = f"{arm}-"
    if not stem.startswith(prefix):
        raise ValueError(
            f"eval dir {eval_sub_dir!r} does not start with its arm {arm!r}. "
            "Results written before the test stem was added to the path cannot "
            "be scored: dev-single-test and devNaive-single-test were writing "
            "to the same directory and overwrote each other."
        )
    return stem[len(prefix):]


class Cell:
    """One (arm x method x eval-test x steer-arm x reps x ablation) cell."""

    def __init__(self, arm, source, base, method, eval_sub_dir, steer_sub_dir,
                 reps, steer_method):
        self.arm = arm
        self.source = source
        self.base = base
        ...
        self.reps = reps
        self.steer_method = steer_method

        self.localization = f"{arm}__from_{source}_to_{base}"
        self.test_stem = _test_stem(eval_sub_dir, arm)
        self.steer_arm = steer_sub_dir[: -len("_steer")]
        self.name = f"{arm} | {self.test_stem} | steer:{self.steer_arm} | {reps} | {steer_method}"

        # reps and steer_method are in the OUTPUT path, not just the filename.
        # Without that, three random seeds and the mean ablation would collapse
        # into one accuracy directory and overwrite each other -- the same class
        # of bug that made the two random baseline modes indistinguishable on
        # disk.
        rel = os.path.join(MODEL_ID, self.localization, method,
                           eval_sub_dir, steer_sub_dir,
                           f"{reps}_{steer_method}")

        self.gen_dir = os.path.join(
            RESULTS_DIR, MODEL_ID, self.localization, method,
            eval_sub_dir, steer_sub_dir, "eval",
        )
        # The filename's trailing token is config.args.test_dataset, which is
        # the ARM, not the test stem -- both test files of one arm share it, and
        # they are kept apart by eval_sub_dir instead.
        self.gen_re = re.compile(
            rf"^(?P<N>\d+)_{re.escape(reps)}_(?P<STEERING_METHOD>{steer_method})_"
            r"(?P<topk>\d+(?:\.\d+)?)_" + re.escape(arm) + r"_gen\.json$"
        )
        self.test_jsonl = os.path.join(
            DATA_DIR, MODEL_ID, arm, f"{self.test_stem}.jsonl"
        )
    ...
    def accuracy_filename(self, n, top_k, name):
        return (f"{n}_{self.reps}_{self.steer_method}_topk_{top_k}"
                f"_gen_accuracy_{name}.json.accuracy.json")

    def __str__(self):
        return f"{self.localization} | {self.eval_sub_dir} | {self.steer_sub_dir} | {self.reps} | {self.steer_method}"


def _keep(value, allowed):
    return not allowed or value in allowed


def all_cells():
    """Walk results/ and build a Cell for every (dir x reps x ablation) found."""
    root = os.path.join(RESULTS_DIR, MODEL_ID)
    if not os.path.isdir(root):
        raise FileNotFoundError(f"no results tree at {root}")
    ...
    for loc in sorted(os.listdir(root)):
        ...
        if not m:
            skipped.append(loc)
            continue
        arm, source, base = m.group("arm"), m.group("source"), m.group("base")
        if not _keep(arm, ARMS):
            continue
        for method in sorted(os.listdir(os.path.join(root, loc))):
            if not _keep(method, METHODS):
                continue
            mdir = os.path.join(root, loc, method)
            if not os.path.isdir(mdir):
                continue
            for ev in sorted(os.listdir(mdir)):
                if not ev.endswith("_eval"):
                    continue
                for st in sorted(os.listdir(os.path.join(mdir, ev))):
                    if not st.endswith("_steer"):
                        continue
                    gd = os.path.join(mdir, ev, st, "eval")
                    ...
                    for fn in os.listdir(gd):
                        ...
                        if not g:
                            continue
                        key = (arm, source, base, method, ev, st,
                               g.group("reps"), g.group("steer_method"))
                        found.setdefault(key, 0)
                        found[key] += 1

    cells = []
    for key in sorted(found):
        arm, source, base, method, ev, st, reps, sm = key
        try:
            stem = _test_stem(ev, arm)
        except ValueError as e:
            skipped.append(f"{ev}: {e}")
            continue
        if not (_keep(stem, EVAL_TESTS) and _keep(st[:-len("_steer")], STEER_ARMS)
                and _keep(reps, REPS_FILTER) and _keep(sm, STEER_METHODS)):
            continue
        cells.append(Cell(arm, source, base, method, ev, st, reps, sm))

    if skipped:
        print(f"  note: skipped {len(skipped)} non-matching entr(ies): "
              f"{skipped[:3]}")
    return cells
    ...
    for gpath in gen_files:
        ...
        for i, item in enumerate(items):
            ...
            record = {
                ...
                "METHOD": cell.method,
                "ARM": cell.arm,
                "TEST_STEM": cell.test_stem,
                "STEER_ARM": cell.steer_arm,
...
def _cell_filename(cell, name, n, top_k):
    # reps and steer_method come from the cell, not module constants: one
    # directory now holds several seeds and both ablation types.
    return cell.accuracy_filename(n, top_k, name)
    ...
    for n in ns:
        for top_k in ks:
            ...
            for name in ACCURACY_METRICS:
                path = os.path.join(cell.accuracy_dir, _cell_filename(cell, name, n, top_k))
...
def main():
    ...
    # Filters on discovery. Each defaults to "everything found on disk", so an
    # unfiltered run scores every cell that exists -- including ones added since
    # the last run, which is the point of discovering rather than enumerating.
    ap.add_argument("--arms", nargs="+", default=None,
                    help="e.g. devuser sysuser sysdev")
    ap.add_argument("--methods", nargs="+", default=None,
                    help="method directory: atp, random-uniform, random-layer_matched")
    ...
    ap.add_argument("--steer_arms", nargs="+", default=None,
                    help="which arm's steering vector was used")
    ap.add_argument("--reps", nargs="+", default=None,
                    help="targeted, or random-s1 random-s2 ...")
    ap.add_argument("--ablations", nargs="+", default=None,
                    choices=["steer", "mean"],
                    help="steer = additive steering, mean = mean-interchange ablation")
    ap.add_argument("--list", action="store_true",
                    help="print the discovered cells and exit, running nothing")
    args = ap.parse_args()

    global ARMS, METHODS, EVAL_TESTS, STEER_ARMS, REPS_FILTER, STEER_METHODS
    ARMS = args.arms or []
    METHODS = args.methods or []
    EVAL_TESTS = args.eval_tests or []
    STEER_ARMS = args.steer_arms or []
    REPS_FILTER = args.reps or []
    STEER_METHODS = args.ablations or []
    ...
    print(f"Family: instruction privilege  model={MODEL_ID}")
    print(f"Running {len(cells)} of {len(all_cells())} cells "
          f"(discovered from {RESULTS_DIR}):")
    for c in cells:
        print(f"  - {c}")
