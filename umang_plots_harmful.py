...
tasks = ["from_harmful-long_to_harmless-long", "from_harmful-single_to_harmless-single"]
task_dict = {
...
steers = ["harmful-long_steer", "harmful-single_steer"]
    ...
    for steer in steers:
        ...
        for top_k in topk_values:
            ...
            for n in steering_factors:
                ...
                jp_s_filename_5 = f"{n}_targeted_steer_topk_{top_k}_gen_accuracy_judge_harmful_5.json.accuracy.json"
                jp_s_filename_4 = f"{n}_targeted_steer_topk_{top_k}_gen_accuracy_judge_harmful_4.json.accuracy.json"
                jp_s_filename_3 = f"{n}_targeted_steer_topk_{top_k}_gen_accuracy_judge_harmful_3.json.accuracy.json"
                ...
                comb_s_filename = f"{n}_targeted_steer_topk_{top_k}_gen_accuracy_comb_harmful.json.accuracy.json"

                jp_r_path_5 = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + jp_r_filename_5
                jp_r_path_4 = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + jp_r_filename_4
                jp_r_path_3 = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + jp_r_filename_3
                jp_s_path_5 = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + jp_s_filename_5
                jp_s_path_4 = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + jp_s_filename_4
                jp_s_path_3 = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + jp_s_filename_3
                flu_path = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + flu_filename
                rel_path = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + rel_filename
                comb_r_path = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + comb_r_filename
                comb_s_path = root_dir + "/" + task + "/" + method + "/harmful-long_eval/" + steer + "/" + comb_s_filename
        ...
        save_dir = f"{RM_INTERP_REPO}/results/plots/{model}/{task}/harmful-long_eval/{steer}/"
        os.makedirs(save_dir, exist_ok=True)
        make_plot(jp_r_df_5, model, task, "harmful-long_eval", steer, "jp_refusal_5", save_dir)
        make_plot(jp_r_df_4, model, task, "harmful-long_eval", steer, "jp_refusal_4", save_dir)
        make_plot(jp_r_df_3, model, task, "harmful-long_eval", steer, "jp_refusal_3", save_dir)
        make_plot(jp_s_df_5, model, task, "harmful-long_eval", steer, "jp_harmful_5", save_dir)
        make_plot(jp_s_df_4, model, task, "harmful-long_eval", steer, "jp_harmful_4", save_dir)
        make_plot(jp_s_df_3, model, task, "harmful-long_eval", steer, "jp_harmful_3", save_dir)
        make_plot(fluency_df, model, task, "harmful-long_eval", steer, "fluency", save_dir)
        make_plot(relevance_df, model, task, "harmful-long_eval", steer, "relevance", save_dir)
        make_plot(comb_r_df, model, task, "harmful-long_eval", steer, "comb_refusal", save_dir)
        make_plot(comb_s_df, model, task, "harmful-long_eval", steer, "comb_harmful", save_dir)
        ...
        for top_k in topk_values:
            ...
            for n in steering_factors:
                ...
                path = root_dir + "/" + task + "/" + method + "/harmful-single_eval/" + steer + "/" + filename
        ...
        print(df)
        save_dir = f"{RM_INTERP_REPO}/results/plots/{model}/{task}/harmful-single_eval/{steer}/"
        os.makedirs(save_dir, exist_ok=True)
        make_plot(df, model, task, "harmful-single_eval", steer, "single", save_dir)
