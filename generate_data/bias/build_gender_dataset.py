...
Output: 8 .jsonl files (single/long x desired/undesired x male/female), each row
indexed by pair idx. "-long-undesired" uses the pair partner's opposite-gender story.
...
def stage_build(professions):
    ...
    print(f"[build] wrote {len(names)} files ({n_pair} rows each) + gender_pairs.json -> {out_dir}", flush=True)
