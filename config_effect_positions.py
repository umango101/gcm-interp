...
class Config:
    ...
    def parse_arguments(self):
        ...
        parser.add_argument('-effect_positions', '--effect_positions', type=str, default='all',
                            choices=['all', 'prompt'],
                            help="Which sequence positions the atp effect sum covers. 'all' (default) "
                                 "sums every position, including the completion region where a "
                                 "question-only source contributes padding rather than a real token. "
                                 "'prompt' restricts the sum to unpadded positions before the response "
                                 "start, so source and base are compared like for like. The base "
                                 "sequence always keeps its completion either way -- the objective L is "
                                 "the log-likelihood of exactly those tokens.")
