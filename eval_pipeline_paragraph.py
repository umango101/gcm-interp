...
For paragraph->sentence that is the full 5 x 2 x 2 x 2 = 40 cells:
    {Falcon3-10B-Instruct, OLMo-2-1124-13B-DPO, Qwen1.5-14B-Chat,
     Qwen1.5-32B-Chat, gemma-3-12b-it}
    x {from_paragraph-long_to_sentence, from_paragraph-single_to_sentence}
...
'single' and 'long' name the RESPONSE FORMAT, not sentence-vs-paragraph:
  ...
  *-long_eval    free generation -> scored by the vLLM 70B judge
                 (length) plus the relevance and fluency judges.
  *-single_eval  4-option MCQA answered with ONE letter -> no judge. Scored as
                 ...
                 one-PARAGRAPH summary of the book named in the question stem.
                 See "MCQA ANSWER KEY" below for how that letter is derived and
                 how the derivation is checked.
...
Run all stages, all cells:   python eval_pipeline_paragraph.py
Run one model:               python eval_pipeline_paragraph.py --models Qwen1.5-14B-Chat
Run a subset of stages:      python eval_pipeline_paragraph.py --stages merge build_prompts
Run only the GPU step:       python eval_pipeline_paragraph.py --stages judge
...
# Every model that has a results/{model}/from_paragraph-*_to_sentence tree.
...
# ('sentence-test.jsonl') -- there is no per-format base in this family.
FAMILY_BASE = "sentence"
...
# answer is the TARGET letter, where the target is the PARAGRAPH (detailed)
# summary of the book named in the question stem. 'mcqa_pre' is the same measure
# on the UNSTEERED answer, i.e. the baseline the steered number is read against.
...
# --- family rule: the target is the PARAGRAPH (detailed) summary -------------
# The four options are {this book, a decoy book} x {one-sentence, one-paragraph}.
# So: split the four into their two book-pairs by topical overlap, keep the pair
# that is about the book named in the stem, and inside it the LONGER option is
# the paragraph summary.
_TITLE_RE = re.compile(r"summary of the book (.+?)\.\s*Please respond", re.S)
_PAIRINGS = [[("A", "B"), ("C", "D")],
             [("A", "C"), ("B", "D")],
             [("A", "D"), ("B", "C")]]
...
def target_letter(content, gold=None):
    """Letter of the one-PARAGRAPH summary of the book the stem asks about.

    `gold` (the test row's own assistant turn, when present) is unused in this
    family and accepted only so both pipelines share a call signature.
    """
    stem, opts = parse_mcqa(content)
    m = _TITLE_RE.search(stem)
    if not m:
        raise ValueError(f"could not find the book title in stem: {stem[:120]!r}")
    title = m.group(1).strip()
    weight = _idf(list(opts.values()) + [title])

    # 1. the two book-pairs: the split with the most within-pair overlap
    pairing = max(_PAIRINGS,
                  key=lambda p: sum(_overlap(opts[x], opts[y], weight) for x, y in p))

    # 2. of those two pairs, the one about THIS book (naming it counts double)
    def title_match(pair):
        named = sum(1.0 for L in pair if title.lower() in opts[L].lower())
        return named + max(_overlap(title, opts[L], weight) for L in pair)

    target_pair = max(pairing, key=title_match)

    # 3. inside that pair, the paragraph summary is the longer of the two
    return max(target_pair, key=lambda L: len(opts[L]))
...
# --- key verification --------------------------------------------------------
# data/{model}/paragraph-single/ carries the same option blocks WITH letters:
# sentence-desired-all.jsonl holds the one-sentence prompt (its assistant turn is
# the sentence letter) and sentence-undesired-all.jsonl the paragraph letter for
# the same id. Re-derive over those and compare.
KEY_CHECK_PROMPTS = "sentence-desired-all.jsonl"
KEY_CHECK_LABELS  = "sentence-undesired-all.jsonl"
...
def main():
    ...
    print(f"Family: paragraph->sentence  method={METHOD}")
