...
#SBATCH -t 48:00:00
#SBATCH -J biasMCQAClassify_experiment
...
declare -a pairs=(
  "femaleMCQAClassify-long_maleMCQAClassify-long"
  "femaleMCQAClassify-single_maleMCQAClassify-single"
)
...
check_data() {
    ...
    for fmt in "${formats[@]}"; do
        needed+=(
            "$DATA_ROOT/$model_name/femaleMCQAClassify-$fmt/maleMCQAClassify-$fmt-test.jsonl"
            "$DATA_ROOT/$model_name/femaleMCQAClassify-$fmt/femaleMCQAClassify-$fmt-steering.jsonl"
            "$DATA_ROOT/$model_name/femaleMCQAClassify-$fmt/maleMCQAClassify-$fmt-steering.jsonl"
        )
    done
    local -a missing=()
    local f
        ...
        for algo in "${algos[@]}"; do
            ...
            for eval_fmt in "${formats[@]}"; do
                ...
                for steer_fmt in "${formats[@]}"; do
                    [[ $STOPPED -eq 1 ]] && break
                    eval_test="$DATA_ROOT/$model_name/femaleMCQAClassify-$eval_fmt/maleMCQAClassify-$eval_fmt-test.jsonl"
                    steer_add="$DATA_ROOT/$model_name/femaleMCQAClassify-$steer_fmt/femaleMCQAClassify-$steer_fmt-steering.jsonl"
                    steer_sub="$DATA_ROOT/$model_name/femaleMCQAClassify-$steer_fmt/maleMCQAClassify-$steer_fmt-steering.jsonl"
