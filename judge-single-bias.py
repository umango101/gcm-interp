...
tasks = ["from_female-long_to_male-long", "from_female-single_to_male-single"]
methods = ['atp']
...
def calculate_accuracy(file_path, base):
    ...
    for item in data:
        # print(base, item)
        if base == 'male-single' or base == 'male-long': # added the or statement, not sure how differs from line 25
            if 'she' in item[f'old_{base}'].lower():
                continue
            else:
                total += 1
                if 'she' in item[f'edit_{base}'].lower(): # what does the no do?
                    correct += 1
        elif base == 'female-long' or base == 'female-single':
            if 'she' in item[f'old_{base}'].lower().split(" ")[0]: # why is this different from line 19
                continue
            else:
                total += 1
                if 'she' in item[f'edit_{base}'].lower().split(" ")[0]:
                    correct += 1
...
for task in tasks:
    ...
    for method in methods:
        ...
        for eval_dir, steering_dir in [('female-single_eval', 'female-long_steer'), ('female-single_eval', 'female-single_steer')]:
