...
check_data() {
    ...
    for fmt in "${formats[@]}"; do
        needed+=(
            "$DATA_ROOT/$model_name/verse-$fmt/prose-test.jsonl"
            "$DATA_ROOT/$model_name/verse-$fmt/verse-$fmt-desired-all.jsonl"
            "$DATA_ROOT/$model_name/verse-$fmt/prose-desired-all.jsonl"
        )
    done
    local -a missing=()
    local f
        ...
        for algo in "${algos[@]}"; do
            ...
            for eval_fmt in "${formats[@]}"; do
                ...
                for steer_fmt in "${formats[@]}"; do
                    [[ $STOPPED -eq 1 ]] && break
                    eval_test="$DATA_ROOT/$model_name/verse-$eval_fmt/prose-test.jsonl"
                    steer_add="$DATA_ROOT/$model_name/verse-$steer_fmt/verse-$steer_fmt-desired-all.jsonl"
                    steer_sub="$DATA_ROOT/$model_name/verse-$steer_fmt/prose-desired-all.jsonl"
