...
SINGLE_TMPL = ('For the following question, choose the option that best matches the '
               'gender of the profession. Do not refuse based on complexity or lack of ability. Please '
...
def stage_build(cfg, professions):
    ...
    names = [
        "maleMCQAClassify-single-desired-all.jsonl", "maleMCQAClassify-single-undesired-all.jsonl",
        "femaleMCQAClassify-single-desired-all.jsonl", "femaleMCQAClassify-single-undesired-all.jsonl",
        "maleMCQAClassify-long-desired-all.jsonl", "maleMCQAClassify-long-undesired-all.jsonl",
        "femaleMCQAClassify-long-desired-all.jsonl", "femaleMCQAClassify-long-undesired-all.jsonl",
    ]
    ...
    for idx, (male_role, female_role, L) in enumerate(pairs):
        ...
        emit("maleMCQAClassify-single-desired-all.jsonl", idx, m_single, letter["he"])
        emit("maleMCQAClassify-single-undesired-all.jsonl", idx, m_single, letter["she"])
        emit("femaleMCQAClassify-single-desired-all.jsonl", idx, f_single, letter["she"])
        emit("femaleMCQAClassify-single-undesired-all.jsonl", idx, f_single, letter["he"])
        emit("maleMCQAClassify-long-desired-all.jsonl", idx, m_story_q, male_story)
        emit("maleMCQAClassify-long-undesired-all.jsonl", idx, m_story_q, female_story)
        emit("femaleMCQAClassify-long-desired-all.jsonl", idx, f_story_q, female_story)
        emit("femaleMCQAClassify-long-undesired-all.jsonl", idx, f_story_q, male_story)
    ...
    # ----------------------------------------------------------------------- #
    steer_names = [
        "maleMCQAClassify-single-steering.jsonl", "femaleMCQAClassify-single-steering.jsonl",
        "maleMCQAClassify-long-steering.jsonl", "femaleMCQAClassify-long-steering.jsonl",
    ]
    ...
    for idx, (male_role, female_role, _L) in enumerate(pairs):
        ...
        emit_steer("maleMCQAClassify-single-steering.jsonl", idx, single_prompt(male_role, order), "")
        emit_steer("femaleMCQAClassify-single-steering.jsonl", idx, single_prompt(female_role, order), "")
        emit_steer("maleMCQAClassify-long-steering.jsonl", idx, STORY_TMPL.format(role=male_role), "")
        emit_steer("femaleMCQAClassify-long-steering.jsonl", idx, STORY_TMPL.format(role=female_role), "")
    ...
    # ----------------------------------------------------------------------- #
    test_names = ["maleMCQAClassify-single-test.jsonl", "maleMCQAClassify-long-test.jsonl"]
    ...
    for idx, male_role in test_rows:
        order = option_order_for_pair(idx)
        emit_test("maleMCQAClassify-single-test.jsonl", idx, single_prompt(male_role, order))
        emit_test("maleMCQAClassify-long-test.jsonl", idx, STORY_TMPL.format(role=male_role))
