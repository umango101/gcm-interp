...
declare -a pairs=(
  "femaleExplicit-long_maleExplicit-long"
...
check_data() {
    ...
    for fmt in "${formats[@]}"; do
        needed+=(
            "$DATA_ROOT/$model_name/femaleExplicit-$fmt/maleExplicit-$fmt-test.jsonl"
            "$DATA_ROOT/$model_name/femaleExplicit-$fmt/femaleExplicit-$fmt-steering.jsonl"
            "$DATA_ROOT/$model_name/femaleExplicit-$fmt/maleExplicit-$fmt-steering.jsonl"
        )
    done
    local -a missing=()
    local f
        ...
        for algo in "${algos[@]}"; do
            ...
            for eval_fmt in "${formats[@]}"; do
                ...
                for steer_fmt in "${formats[@]}"; do
                    [[ $STOPPED -eq 1 ]] && break
                    eval_test="$DATA_ROOT/$model_name/femaleExplicit-$eval_fmt/maleExplicit-$eval_fmt-test.jsonl"
                    steer_add="$DATA_ROOT/$model_name/femaleExplicit-$steer_fmt/femaleExplicit-$steer_fmt-steering.jsonl"
                    steer_sub="$DATA_ROOT/$model_name/femaleExplicit-$steer_fmt/maleExplicit-$steer_fmt-steering.jsonl"
